//
//  ResultScene.swift
//  Geneventure
//
//  Created by Benedictus Yogatama Favian Satyajati on 02/03/26.
//

import SwiftUI

struct ResultView: View {
    let levelData: LevelData
    let onContinue: () -> Void
    let onReplay: () -> Void
    @State private var appeared = false
    @State private var breathing = false
    @State private var showInfiniteUnlockAlert = false
    @ObservedObject private var gameState = GameState.shared
    @Environment(\.horizontalSizeClass) private var sizeClass

    private var isCompact: Bool { sizeClass == .compact }

    var body: some View {
        ZStack {
            Image("bg_purple")
                .interpolation(.none)
                .resizable()
                .scaledToFill()
                .ignoresSafeArea()

            if isCompact {
                VStack(spacing: 32) {
                    starImage

                    textBlock

                    continueButton
                }
                .padding(.horizontal, 24)
            } else {
                HStack(spacing: 80) {
                    starImage

                    VStack(spacing: 40) {
                        textBlock
                        continueButton
                    }
                }
                .padding(.horizontal, 32)
            }
        }
        .onAppear {
            appeared = true
            if levelData.id == 10 && !gameState.hasSeenInfiniteUnlock {
                showInfiniteUnlockAlert = true
            }
        }
        .alert("Infinite Mode Unlocked!", isPresented: $showInfiniteUnlockAlert) {
            Button("Awesome!") {
                gameState.setSeenInfiniteUnlock()
            }
        } message: {
            Text("You've completed all levels and unlocked Infinite Mode! You can now access it from the main menu.")
        }
    }

    private var starImage: some View {
        Image("star")
            .interpolation(.none)
            .resizable()
            .scaledToFit()
            .frame(
                minWidth: isCompact ? 100 : 140,
                maxWidth: isCompact ? 160 : 220,
                minHeight: isCompact ? 100 : 140,
                maxHeight: isCompact ? 160 : 220
            )
            .shadow(color: .yellow.opacity(0.5), radius: 20)
            .scaleEffect(appeared ? 1.0 : 0.3)
            .animation(.spring(response: 0.5, dampingFraction: 0.6), value: appeared)
    }

    private var textBlock: some View {
        VStack(spacing: isCompact ? 4 : 8) {
            Text("Level Complete!")
                .font(.system(size: isCompact ? 32 : 44, weight: .black, design: .rounded))
                .foregroundColor(.white)
                .shadow(color: .black.opacity(0.3), radius: 2, y: 2)
                .multilineTextAlignment(.center)

            Text(levelData.title)
                .font(.system(size: isCompact ? 18 : 24, weight: .semibold, design: .rounded))
                .foregroundColor(.white.opacity(0.85))
                .shadow(color: .black.opacity(0.2), radius: 1, y: 1)
                .multilineTextAlignment(.center)
        }
        .opacity(appeared ? 1 : 0)
        .offset(y: appeared ? 0 : 20)
        .animation(.easeOut(duration: 0.4).delay(0.2), value: appeared)
    }

    private var continueButton: some View {
        Button(action: onContinue) {
            ZStack {
                Image("genotype_card_white_thin")
                    .interpolation(.none)
                    .resizable()
                    .scaledToFit()
                    .frame(maxWidth: isCompact ? 200 : 240)
                    .shadow(color: .black.opacity(0.2), radius: 6, y: 4)

                HStack(spacing: 8) {
                    Text("Continue")
                        .font(.system(size: isCompact ? 17 : 20, weight: .bold, design: .rounded))
                        .foregroundColor(.black)
                    Image("arrow")
                        .interpolation(.none)
                        .resizable()
                        .scaledToFit()
                        .frame(height: 16)
                }
            }
        }
        .scaleEffect(breathing ? 1.05 : 0.97)
        .onAppear {
            withAnimation(.easeInOut(duration: 0.9).repeatForever(autoreverses: true)) {
                breathing = true
            }
        }
        .opacity(appeared ? 1 : 0)
        .animation(.easeOut(duration: 0.4).delay(0.4), value: appeared)
    }
}
